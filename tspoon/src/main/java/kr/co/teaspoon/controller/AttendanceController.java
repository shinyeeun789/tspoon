...
public class AttendanceController {
    ...
    public String addAttend(HttpServletRequest request, RedirectAttributes rttr) throws Exception {
        ...
        return "redirect:/attendance/check.do";
    }

    @GetMapping("list.do")
    public String attendList(HttpServletRequest request, Model model) throws Exception {
        HttpSession session = request.getSession();
        String id = (String) session.getAttribute("sid");
        List<Attendance> attendances = attendanceService.attendaceDetail(id);
